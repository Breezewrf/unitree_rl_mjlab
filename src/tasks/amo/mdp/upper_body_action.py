...
class UpperBodyMotionAction(ActionTerm):
  ...
  def apply_actions(self) -> None:
    ...
    if not hasattr(self._env, "_upper_body_kp_kd_printed"):
      self._env._upper_body_kp_kd_printed = True
      print("[UpperBodyMotion] kp/kd per joint:")
      for i, jid in enumerate(self._joint_ids):
        jname = self._entity.joint_names[jid]
        for act in self._entity.actuators:
          cfg = act.cfg
          if hasattr(cfg, "target_names_expr"):
            if any(re.search(p, jname) for p in cfg.target_names_expr):
              print(f"  {jname}: kp={cfg.stiffness:.2f}, kd={cfg.damping:.2f}")
              break
